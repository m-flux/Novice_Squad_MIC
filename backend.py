...
def send_proposals(body: ProposalSend):
    ...
    If no solo companies matched AND is_collaborative is True,
    create a collaboration entry instead.
    """
    created_proposals = []
    created_collab = None

    # Case 1: Solo proposals to specific companies
    ...
    # Case 2: No solo matches & collaborative flag → create collaboration
    if not body.target_company_ids and body.is_collaborative:
...
def company_dashboard(company_id: str):
    ...
    for _, row in df_collab[df_collab["Status"] == "Active"].iterrows():
        if row["Domain"].strip() in company_domains:
            d = row.to_dict()
            d["is_priority_region"] = is_priority_region(str(row["Region"]))
